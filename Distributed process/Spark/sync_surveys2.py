...
to_create_df = spark.read.schema(id_schema).json("/tmp/to_create.json")
to_update_df = spark.read.schema(id_schema).json("/tmp/to_update.json")
...
# 3. Recharger la table surveys et parser le champ 'doc'
surveys = spark.table("surveys_nosql_ext")

# 4. Join avec la liste d'ID pour récupérer les documents concernés
docs_to_process = all_ids.join(surveys, on="id", how="inner").select("doc")

# 5. Sauvegarde en NDJSON (JSONL)
docs_to_process.write.mode("overwrite").text("/tmp/surveys_for_mapreduce.jsonl")
